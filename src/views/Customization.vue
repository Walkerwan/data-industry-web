  ...
  <div class="customization">
    ...
    <div class="content-area">
      <div class="combination-method-wrapper">
        <div class="flex-row combination-method title">
          ...
          <Button @click="modal = true" class="clearAll" :disabled="notEditable">
            ...
            <span style="vertical-align: text-bottom">全部清除</span></Button>
          <Modal
            v-model="modal"
            title="提示"
            @on-ok="ok"
        ...
        <div class="flex-row tabs-wrapper">
          ...
          <div class="flex-row tabs">
            <div v-for="(crowd, index) in crowds" :id="'crowd-group-tab-'+crowd.groupName" class="flex-row tab"
              ...
              <Button :disabled="notEditable" type="primary" shape="circle"
                      @click="changeCombinationMethodToPeer(index)"
                      class="crowd-group-operation" v-if="index<4">{{combinationMethodName[crowd.outsideRelationType]}}
              </Button>
            </div>
          </div>
            ...
            <div class="tag-table">
              ...
              <div class="tag-table-body">
                <div v-for="(tag,index) in currentCrowd.selectedTags" :id="'current-crowd-tag-'+index"
                     class="tag-table-body-row" :draggable="!notEditable" @dragstart="tagDrag($event,index)"
      ...
      <div class="form-area flex-row">
        <div class="form">
          <Form :model="form" :rules="rules" ref="form" style="width: 400px;" :label-width="100" @submit.native.prevent>
            ...
            <FormItem label="人群名称：" prop="customizedCrowdName">
              <Input class="crowd-name-input" v-model="form.customizedCrowdName" :disabled="notEditable"
                     placeholder="请输入人群名称"/>
            </FormItem>
          </Form>
        </div>
        <div class="buttons">
          <Button type="primary" @click.stop="submitComstomization" :disabled="notEditable">提交</Button>
...
<script>
  ...
  function transferDataProperty(source = {}) {
    ...
    newArr.forEach(item => {
      source.industryLabels.push({
        indLabelId: item.indLabelId,
        indLabelCode: item.indLabelCode,
        indLabelName: item.indLabelName,
        peoScaleNum: item.peoScaleNum,
        indInvolvedGroupid: item.indInvolvedGroupid,
        indInvolvedGroupidpath: item.indInvolvedGroupidpath,
        indInvolvedGroupnamepath: item.indInvolvedGroupnamepath
      });
    });
  }
  ...
  export default {
    ...
    methods: {
          ...
          if (valid) {
            ...
            this.$store
              ...
              .then(res => {
                ...
                this.$router.push({path: "/crowdManage"});
              });
          } else {
      ...
      deleteCrowd(delectCrowd) {
        this.$store.commit("crowdEdit/deleteCrowd", delectCrowd);
      },
      addCrowd() {
      ...
      tagDrag(e, index) {
        !this.notEditable && e.dataTransfer.setData("draggedtagindex", index);
      },
      tagDrop(e, index) {
        e.preventDefault();
      ...
      newArrayAfterDragDrop(draggedIndex, droppedIndex, objArr) {
        console.log(draggedIndex, droppedIndex, objArr);
